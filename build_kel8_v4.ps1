...
$tmpDir     = 'C:\Users\noval mulyadi\.gemini\antigravity\scratch\goplay\docx_kel8_v4'
...
function h1([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"H1`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function h2([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"H2`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function h3([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"H3`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function pp([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"Normal`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function pc([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"CTR`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function ph([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"PH`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function bd([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"Normal`"/></w:pPr><w:r><w:rPr><w:b/></w:rPr><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function li([string]$t) { $e = XE $t; ap "<w:p><w:pPr><w:pStyle w:val=`"LI`"/></w:pPr><w:r><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
function gj([string]$num,[string]$cap) { $e = XE "Gambar $num $cap"; ap "<w:p><w:pPr><w:pStyle w:val=`"CAP`"/></w:pPr><w:r><w:rPr><w:i/></w:rPr><w:t xml:space=`"preserve`">$e</w:t></w:r></w:p>" }
...
pp "         2.3.3  SQLite"
pp "         2.3.4  Vite"
pp "    2.4  Metodologi Pengembangan Perangkat Lunak (Agile Scrum)"
...
pp "Gambar 5.5   Screenshot Dashboard Owner"
pp "Gambar 5.6   Screenshot Monitoring Timer Unit"
pp "Gambar 5.7   Screenshot Kelola Reservasi Owner"
...
pp "4. Kesulitan Rekapitulasi Data: Owner kesulitan dalam melakukan audit pendapatan harian maupun bulanan karena data tidak tersimpan dalam basis data yang terstruktur dan aman."
...
pp "4. Menghasilkan Laporan Pendapatan yang transparan dan akurat guna memudahkan manajemen usaha."
...
pp "2. Aktor Sistem: Owner (pengelola operasional dan pemantau laporan) dan Member (pelanggan terdaftar)."
...
pp "Bagi Owner: Kemudahan memantau operasional, mempercepat pelayanan transaksi, mempermudah kontrol unit, mengurangi beban kerja administratif, serta keamanan data finansial melalui laporan pendapatan yang terstruktur."
...
pp "Operasional GO PLAY didukung oleh Owner yang mengawasi operasional harian, layanan transaksi, arus kas, dan kebijakan bisnis. Meskipun memiliki basis pelanggan yang stabil di wilayah Subang, GO PLAY masih menghadapi kendala efisiensi akibat penggunaan buku besar dalam pencatatan transaksi."
...
pp "Aplikasi berbasis web adalah perangkat lunak yang diakses melalui penjelajah web (web browser) melalui jaringan internet atau intranet. Keunggulan utama adalah sifatnya yang multi-platform, sehingga dapat diakses oleh Member maupun Owner melalui berbagai perangkat tanpa perlu instalasi tambahan."
...
h3 "2.3.2 Laravel 12"
pp "Laravel adalah framework PHP open-source yang menggunakan pola desain MVC (Model-View-Controller). Laravel 12 digunakan sebagai backbone backend sistem GO PLAY karena kemampuannya dalam menangani routing yang kompleks, sistem autentikasi, manajemen database melalui Eloquent ORM, serta fitur keamanan bawaan seperti proteksi CSRF dan enkripsi password menggunakan Bcrypt."
pp "Komponen MVC dalam Laravel:"
pp "Model    : Mengelola interaksi dengan database (tabel users, consoles, bookings, transactions)."
pp "View     : Mengelola tampilan antarmuka menggunakan Blade Template Engine."
pp "Controller : Menghubungkan logika bisnis antara Model dan View, dipisahkan berdasarkan peran."

h3 "2.3.3 SQLite"
pp "SQLite adalah sistem manajemen basis data relasional berbasis file yang ringan dan tidak memerlukan server database terpisah. SQLite dipilih untuk pengembangan sistem GO PLAY karena kemudahannya dalam setup dan portabilitasnya."

h3 "2.3.4 Vite"
pp "Vite adalah build tool frontend generasi terbaru yang digunakan sebagai pengganti Laravel Mix. Vite menyediakan fitur Hot Module Replacement (HMR) yang memberikan pembaruan tampilan secara instan selama proses pengembangan."

h2 "2.4 Metodologi Pengembangan Perangkat Lunak (Agile Scrum)"
pp "Pengembangan sistem GO PLAY menggunakan metodologi Agile Scrum, yang memungkinkan pengembangan dilakukan secara iteratif dan adaptif terhadap perubahan kebutuhan. Tahapan dalam Scrum meliputi:"
pp "1. Product Backlog: Menyusun daftar seluruh kebutuhan fungsional berdasarkan hasil analisis SRS, seperti fitur Login, Booking Online, hingga Laporan Pendapatan."
pp "2. Sprint Planning: Menentukan daftar tugas (backlog) yang akan dikerjakan dalam satu iterasi (Sprint)."
pp "3. Daily Scrum: Pertemuan harian untuk memantau progres pengembangan dan mengidentifikasi hambatan teknis."
pp "4. Sprint Review: Sesi demonstrasi hasil fitur yang telah selesai dibangun kepada stakeholder untuk mendapatkan umpan balik langsung."
pp "5. Sprint Retrospective: Melakukan evaluasi internal terhadap performa tim selama Sprint berlangsung guna meningkatkan kualitas kerja."
...
pp "Jalur Offline (Walk-in): Pelanggan datang langsung ke lokasi (tanpa aplikasi). Owner menerima kedatangan tersebut dan langsung memprosesnya melalui jalur Input Langsung."
...
pp "Validasi Otomatis (Sistem): Untuk pesanan online, Sistem secara otomatis menjalankan fungsi Mengecek Ketersediaan Jadwal. Jika Penuh: alur dikembalikan ke Member. Jika Tersedia: pesanan diteruskan ke Owner."
pp "Validasi Manual (Owner): Untuk jalur offline, Owner langsung menuju fitur F-05 untuk mengecek ketersediaan unit secara manual di tempat."

h3 "3.1.3 Tahap Pengelolaan Reservasi dan Persiapan (Owner)"
pp "F-05: Kelola Reservasi: Owner mengonfirmasi booking online yang masuk atau melakukan input manual untuk pelanggan offline."
pp "F-03: Kelola Inventaris: Owner melakukan pengecekan fisik pada perangkat (konsol PS dan controller) untuk memastikan kondisi baik dan layak pakai."

h3 "3.1.4 Tahap Operasional dan Monitoring"
pp "F-06: Aktifkan Timer dan Monitoring: Setelah perangkat siap dan pelanggan mulai bermain, Owner mengaktifkan fitur timer untuk memantau durasi bermain dan memberikan notifikasi sisa waktu secara otomatis."

h3 "3.1.5 Tahap Pelaporan (Output Akhir)"
pp "F-07: Laporan Pendapatan: Setelah sesi bermain selesai dan transaksi ditutup, data secara otomatis mengalir untuk rekapitulasi keuangan."
...
ph "[TEMPEL TABEL KEBUTUHAN FUNGSIONAL (F-01 sd F-07) DI SINI]"
el
pp "F-01 Login              : Memvalidasi kredensial pengguna. Aktor: Owner, Member. Prioritas: Tinggi."
pp "F-02 Log Out            : Mengakhiri sesi pengguna secara aman. Aktor: Owner, Member. Prioritas: Tinggi."
pp "F-03 Kelola Inventaris  : Mengelola data unit PS4/PS5 dan kelayakan controller. Aktor: Owner. Prioritas: Sedang."
pp "F-04 Booking Online     : Pemesanan jadwal dan unit secara mandiri. Aktor: Member. Prioritas: Tinggi."
pp "F-05 Kelola Reservasi   : Validasi, konfirmasi, atau ubah data reservasi masuk. Aktor: Owner. Prioritas: Tinggi."
pp "F-06 Monitoring Unit    : Pantau status unit, timer otomatis, notifikasi sisa waktu. Aktor: Owner, Member. Prioritas: Tinggi."
pp "F-07 Laporan Pendapatan : Rekapitulasi keuangan transaksi selesai. Aktor: Owner. Prioritas: Sedang."
el
bd "Tabel 3.3 Kebutuhan Non-Fungsional (Non-Functional Requirements)"
ph "[TEMPEL TABEL KEBUTUHAN NON-FUNGSIONAL (NF-01 sd NF-06) DI SINI]"
el
...
pp "Skenario Utama: (1) Owner memilih menu Kelola Inventaris. (2) Sistem menampilkan daftar unit. (3) Owner menambah, mengubah status (baik/rusak), atau menghapus unit. (4) Owner menyimpan perubahan. (5) Sistem memperbarui database."
el
...
pp "Skenario Utama: (1) Owner masuk ke menu Kelola Reservasi. (2) Sistem menampilkan daftar pesanan. (3) Owner melakukan verifikasi. (4) Owner mengubah status (Konfirmasi/Dibatalkan/Selesai). (5) Sistem memperbarui status jadwal."
el
bd "6. Use Case: Monitoring Status Unit"
pp "Aktor: Owner, Member"
pp "Deskripsi: Memantau sisa waktu bermain dan status unit secara real-time."
pp "Skenario Utama: (1) Aktor membuka dashboard monitoring. (2) Sistem menampilkan status unit (Kosong/Terisi/Dipesan). (3) Sistem menjalankan timer otomatis. (4) Sistem memberikan notifikasi jika waktu hampir habis. (5) Owner dapat menghentikan atau menambah durasi billing."
el
bd "7. Use Case: Laporan Pendapatan"
pp "Aktor: Owner"
pp "Deskripsi: Melihat hasil rekapitulasi keuangan dari transaksi sewa."
pp "Skenario Utama: (1) Owner masuk ke menu Laporan Pendapatan. (2) Owner menentukan filter periode (Harian/Bulanan). (3) Sistem menarik data transaksi berstatus Selesai. (4) Sistem menampilkan total pendapatan dan detail. (5) Owner dapat mengunduh atau mencetak laporan."
...
pp "Model      : Mengelola data dan interaksi dengan database SQLite. Model utama: User, Console, Booking, Transaction."
...
pp "Controller : Logika bisnis aplikasi, memproses request pengguna, berinteraksi dengan Model, dan mengembalikan respons ke View. Dipisahkan berdasarkan peran: AuthController, BookingController (Member), BookingAdminController, ConsoleController, ReportController (Owner)."
...
el
bd "Tabel users (Data Member/Owner)"
pp "id (PK), name, email (unique), password (encrypted Bcrypt), role (owner/member), phone, email_verified_at, remember_token, created_at, updated_at."
el
...
pp "Seeder yang disediakan: data Owner default (owner@goplay.com), data konsol PS4/PS5 (5 unit), dan data Member contoh untuk keperluan pengujian sistem."
...
h3 "5.2.3 Dashboard dan Fitur Owner"
ph "[TEMPEL GAMBAR 5.5 - SCREENSHOT DASHBOARD OWNER DARI USER GUIDE DI SINI]"
gj "5.5" "Screenshot Dashboard Owner GO PLAY"
...
gj "5.7" "Screenshot Kelola Reservasi Owner"
...
pp "app/Http/Controllers/  - Semua Controller (Owner/, Member/, Auth/)"
pp "app/Models/            - Semua Model (User, Console, Booking, Transaction)"
...
pp "4. Akses halaman Owner oleh Member - sistem melakukan redirect ke halaman yang sesuai."
el
ph "[TEMPEL TABEL/SCREENSHOT HASIL PENGUJIAN DARI DOKUMENTASI PENGUJIAN DI SINI]"

h2 "5.5 User Acceptance Testing (UAT)"
pp "User Acceptance Testing (UAT) dilakukan dengan melibatkan pengguna nyata (Owner dan Member GO PLAY) untuk memastikan sistem telah memenuhi kebutuhan dan ekspektasi pengguna akhir. UAT dilakukan berdasarkan skenario pengujian yang disusun dari setiap use case yang telah didefinisikan."
el
...
pp "4. Fitur Timer Otomatis (F-06) berhasil memantau durasi bermain secara real-time dan memberikan notifikasi saat waktu hampir habis, sehingga mengurangi beban kerja."
pp "5. Dashboard Laporan Pendapatan (F-07) menyajikan rekapitulasi keuangan yang akurat, dapat difilter harian maupun bulanan, memudahkan Owner dalam audit keuangan."
