            ...
            <div class="bk-unit-outer">
                ...
                <div class="bk-unit-inner">
                    <div class="bk-console-grid">
                        ...
                        @endphp
                        <div class="bk-console-card {{ $isBooked ? 'disabled' : '' }} {{ old('console_id') == $console->id ? 'selected' : '' }}"
                             onclick="{{ $isBooked ? 'alert(\'Unit ini sudah dipesan\')' : "selectConsole({$console->id}, '{$console->name}', {$price}, '{$imgKey}', this)" }}">
                            <div class="bk-card-inner">
                                <div class="bk-card-img">
                                    <img src="{{ asset('images/'.$imgKey.'.png') }}"
                                         alt="{{ $console->name }}"
                                         class="{{ $isPs5 ? 'ps5' : 'ps4' }}">
                                </div>
                                <div class="bk-card-name">{{ $console->name }}</div>
                                <div class="bk-card-status {{ $isBooked ? 'reserved' : 'available' }}">
                                    {{ $isBooked ? '● Reserved' : '● Tersedia' }}
                                </div>
                ...
                <div class="bk-sum-body">
                    <div class="bk-sum-console">
                        <div class="bk-sum-imgbox">
                            <img src="{{ asset('images/ps5.png') }}" alt="console" id="summaryImg">
                        </div>
                        <div>
                    ...
                    <button type="submit" class="bk-confirm-btn">🎮 Konfirmasi Booking</button>
...
@if(isset($defaultConsole))
document.addEventListener('DOMContentLoaded', () => {
    const defaultCard = document.querySelector('.bk-console-card[onclick*="selectConsole({{ $defaultConsole->id }},"]');
    if (defaultCard && !defaultCard.classList.contains('disabled')) {
        defaultCard.click();
    }
    
    @if(isset($defaultTime))
    const defaultTimeSlot = document.querySelector('.bk-time-slot[onclick*="selectTime(\'{{ $defaultTime }}\'"]');
    if (defaultTimeSlot) {
...
function selectConsole(id, name, price, img, el) {
    ...
    document.getElementById('summaryImg').src = `/images/${img}.png`;
