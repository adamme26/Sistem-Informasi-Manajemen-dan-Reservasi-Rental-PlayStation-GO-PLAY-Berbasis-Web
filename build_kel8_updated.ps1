...
$tmpDir = 'C:\Users\noval mulyadi\.gemini\antigravity\scratch\goplay\docx_kel8'
...
function WF($path, $content) { [System.IO.File]::WriteAllText($path, $content, [System.Text.Encoding]::UTF8) }

WF "$tmpDir\[Content_Types].xml" '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/><Override PartName="/word/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml"/></Types>'
WF "$tmpDir\_rels\.rels" '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/></Relationships>'
WF "$tmpDir\word\_rels\document.xml.rels" '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>'
...
<w:style w:type="paragraph" w:styleId="CAP"><w:name w:val="CAP"/><w:basedOn w:val="Normal"/><w:pPr><w:jc w:val="center"/></w:pPr><w:rPr><w:i/><w:sz w:val="20"/></w:rPr></w:style>
...
function ap([string]$x) { $sb.Append($x) | Out-Null }
...
function li([string]$t) { ap "<w:p><w:pPr><w:pStyle w:val='Normal'/><w:ind w:left='720' w:hanging='360'/></w:pPr><w:r><w:t xml:space='preserve'>$t</w:t></w:r></w:p>" }
...
pp "2. Lemahnya Transparansi Informasi: Member tidak memiliki akses langsung untuk memantau status ketersediaan unit secara real-time, sehingga proses reservasi menjadi tidak efektif."
...
pp "1. Fungsionalitas Utama: Meliputi manajemen akun (Login & Log Out), pengelolaan inventaris (PS4/PS5), sistem reservasi online, monitoring status unit (real-time timer), dan pelaporan pendapatan harian/bulanan."
pp "2. Aktor Sistem: Terdiri dari tiga level pengguna sesuai hak aksesnya, yaitu Admin (pengelola operasional), Pemilik (pemantau laporan), dan Member (pelanggan terdaftar)."
pp "3. Teknologi: Aplikasi dikembangkan menggunakan framework Laravel 12 (PHP 8.2) dengan database SQLite dan frontend Vite + Blade."
pp "4. Batasan Sistem: Sistem ini tidak mencakup kontrol perangkat keras (mematikan mesin PS secara otomatis) dan tidak mendukung integrasi payment gateway (pembayaran dilakukan secara manual dan dicatat oleh sistem)."

h2 "1.5 Manfaat yang Diharapkan"
pp "Bagi Pemilik (Owner): Memberikan kemudahan dalam memantau kinerja bisnis secara real-time serta menjamin keamanan data finansial melalui laporan pendapatan yang terstruktur."
...
pp "Bagi Member (Pelanggan): Memberikan pengalaman pemesanan yang lebih modern, cepat, serta transparansi biaya dan waktu bermain yang lebih jelas."
pp "Bagi Pengembang: Sebagai sarana penerapan ilmu rekayasa perangkat lunak dalam membangun sistem informasi berbasis web menggunakan framework modern."

# ================================================================ BAB II (DIPERBAIKI - teknologi diupdate ke Laravel/SQLite)
pb
h1 "BAB II"
...
pp "Saat ini, operasional GO PLAY didukung oleh dua aktor utama: Pemilik (Owner) yang mengawasi arus kas dan kebijakan bisnis, serta Admin yang menangani layanan transaksi harian. Meskipun memiliki basis pelanggan yang stabil di wilayah Subang, GO PLAY masih menghadapi kendala efisiensi akibat penggunaan buku besar dalam pencatatan transaksi. Hal ini mendorong kebutuhan akan digitalisasi sistem untuk meningkatkan transparansi dan akurasi data."
...
pp "Aplikasi berbasis web adalah perangkat lunak yang diakses melalui penjelajah web (web browser) melalui jaringan internet atau intranet. Keunggulan utama aplikasi web adalah sifatnya yang multi-platform, sehingga dapat diakses oleh Member maupun Admin melalui berbagai perangkat seperti smartphone, laptop, maupun tablet tanpa perlu instalasi tambahan."
...
pp "PHP adalah bahasa pemrograman server-side yang dirancang khusus untuk pengembangan web. Proyek ini menggunakan PHP versi 8.2 yang menawarkan performa lebih cepat, fitur keamanan terbaru, dan type-hinting yang lebih baik untuk menjaga stabilitas sistem billing. Fitur type-hinting yang ketat ini mewajibkan pengembang untuk memastikan tipe data yang tepat, seperti penggunaan casting (int) pada parameter durasi pemesanan."

h3 "2.3.2 Laravel 12"
pp "Laravel adalah framework PHP open-source yang menggunakan pola desain MVC (Model-View-Controller). Laravel 12 digunakan sebagai backbone backend sistem GO PLAY karena kemampuannya dalam menangani routing yang kompleks, sistem autentikasi multi-guard (untuk membedakan Admin, Member, dan Pemilik), manajemen database melalui Eloquent ORM, serta fitur keamanan bawaan seperti proteksi CSRF dan enkripsi password menggunakan Bcrypt."
...
h3 "2.3.3 SQLite"
pp "SQLite adalah sistem manajemen basis data relasional berbasis file yang ringan dan tidak memerlukan server database terpisah. SQLite dipilih untuk pengembangan dan pengujian lokal sistem GO PLAY karena kemudahannya dalam setup dan portabilitasnya. Seluruh data sistem tersimpan dalam satu file database.sqlite yang dapat dengan mudah dipindahkan atau di-backup."

h3 "2.3.4 Vite"
pp "Vite adalah build tool frontend generasi terbaru yang digunakan sebagai pengganti Laravel Mix. Vite menyediakan fitur Hot Module Replacement (HMR) yang memberikan pembaruan tampilan secara instan selama proses pengembangan tanpa perlu me-refresh browser secara manual, sehingga meningkatkan efisiensi proses pengembangan antarmuka."

h2 "2.4 Metodologi Pengembangan Perangkat Lunak (Waterfall)"
pp "Pengembangan sistem GO PLAY menggunakan metodologi Waterfall, yaitu pendekatan pengembangan perangkat lunak yang bersifat sekuensial dan terstruktur. Setiap fase harus diselesaikan sepenuhnya sebelum melanjutkan ke fase berikutnya. Tahapan dalam metodologi Waterfall meliputi:"
pp "1. Analisis Kebutuhan: Mengidentifikasi dan mendokumentasikan seluruh kebutuhan fungsional (F-01 sd F-07) dan non-fungsional (NF-01 sd NF-06) sistem berdasarkan wawancara dengan pemilik GO PLAY dan analisis sistem manual yang sedang berjalan."
pp "2. Perancangan Sistem: Merancang arsitektur sistem (MVC), struktur basis data (ERD), pemodelan UML (Use Case, Activity, Class, Sequence Diagram), dan desain antarmuka pengguna (Wireframe Figma)."
...
pp "4. Pengujian: Melakukan pengujian fungsional (Black Box Testing) dan User Acceptance Testing (UAT) untuk memastikan sistem berjalan sesuai spesifikasi."
pp "5. Pemeliharaan: Melakukan perbaikan bug dan penyesuaian sistem jika ditemukan kesalahan pasca-pengujian."
...
pp "Use Case Diagram: Menggambarkan interaksi antara aktor (Admin, Member, Pemilik) dengan fungsionalitas sistem."
pp "Activity Diagram: Menggambarkan aliran kerja (workflow) dari setiap proses bisnis."
pp "Robustness Diagram: Menggambarkan hubungan antara aktor, boundary, control, dan entity dalam sistem."
pp "Class Diagram: Menggambarkan struktur kelas dan hubungan antar objek dalam database."
pp "Sequence Diagram: Menggambarkan urutan interaksi antar objek dalam sistem untuk setiap skenario use case."

# ================================================================ BAB III (DIPERTAHANKAN dari dokumen asli + penyesuaian)
pb
h1 "BAB III"
h1 "ANALISIS SISTEM"
...
pp "Diagram ini menunjukkan bahwa proses dapat dimulai dari dua titik berbeda tergantung pada cara pelanggan datang:"
...
ph "[TEMPEL GAMBAR 3.1 - DIAGRAM BPMN PROSES BISNIS GO PLAY DI SINI]"
gj "3.1" "BPMN Proses Bisnis Sistem GO PLAY"

h3 "3.1.2 Tahap Validasi dan Pengecekan"
pp "Pada tahap ini, sistem dan Admin bekerja untuk memastikan ketersediaan:"
pp "Validasi Otomatis (Sistem): Untuk pesanan online, Sistem secara otomatis menjalankan fungsi Mengecek Ketersediaan Jadwal."
pp "Jika Penuh (TIDAK): Alur akan dikembalikan ke Member pada fitur F-04 untuk memilih jadwal atau unit lain."
pp "Jika Tersedia (YA): Pesanan diteruskan ke Admin dan masuk ke dalam antrean reservasi."
...
pp "Setelah jadwal dipastikan tersedia, Admin mengambil alih kendali penuh:"
...
pp "F-03: Kelola Inventaris: Sebelum permainan dimulai, Admin melakukan pengecekan fisik pada perangkat (konsol PS dan controller) untuk memastikan semuanya dalam kondisi baik dan layak pakai."

h3 "3.1.4 Tahap Operasional dan Monitoring"
pp "F-06: Aktifkan Timer dan Monitoring: Setelah perangkat siap dan pelanggan mulai bermain, Admin mengaktifkan fitur timer. Fitur ini berfungsi untuk memantau durasi bermain dan memberikan notifikasi sisa waktu secara otomatis."

h3 "3.1.5 Tahap Pelaporan (Output Akhir)"
pp "F-07: Laporan Pendapatan: Setelah sesi bermain selesai dan transaksi ditutup, data tersebut secara otomatis mengalir ke lajur Pemilik dalam bentuk laporan pendapatan. Ini memudahkan Pemilik untuk memantau rekapitulasi keuangan berdasarkan transaksi yang sudah selesai."
...
pp "Berdasarkan dokumen SRS, kebutuhan sistem dirinci menjadi:"
el
bd "Kebutuhan Fungsional (Functional):"
bd "Tabel 3.2 Spesifikasi Kebutuhan Fungsional"
ph "[TEMPEL TABEL KEBUTUHAN FUNGSIONAL (F-01 sd F-07) DARI FILE Dokumen SRS_Daftar Kebutuhan_Sistem GOPLAY.pdf DI SINI]"
el
pp "F-01 Login: Memvalidasi kredensial pengguna untuk masuk ke dalam sistem sesuai hak akses. Aktor: Admin, Pemilik, Member. Prioritas: Tinggi."
pp "F-02 Log Out: Mengakhiri sesi pengguna pada sistem secara aman. Aktor: Admin, Pemilik, Member. Prioritas: Tinggi."
pp "F-03 Kelola Inventaris: Mengelola data unit (PS4/PS5) dan memantau kelayakan perangkat/controller. Aktor: Admin. Prioritas: Sedang."
pp "F-04 Booking Online: Melakukan pemesanan jadwal dan unit secara mandiri melalui aplikasi. Aktor: Member. Prioritas: Tinggi."
pp "F-05 Kelola Reservasi: Memvalidasi ketersediaan unit, mengonfirmasi, atau mengubah data reservasi masuk. Aktor: Admin. Prioritas: Tinggi."
pp "F-06 Monitoring Status Unit: Memantau status unit (Kosong/Isi), menjalankan timer otomatis, dan notifikasi sisa waktu. Aktor: Admin, Member. Prioritas: Tinggi."
pp "F-07 Laporan Pendapatan: Menyusun dan menampilkan rekapitulasi keuangan berdasarkan transaksi yang selesai. Aktor: Admin, Pemilik. Prioritas: Sedang."
el
bd "Kebutuhan Non-Fungsional (Non-Functional):"
bd "Tabel 3.3 Spesifikasi Kebutuhan Non-Fungsional"
ph "[TEMPEL TABEL KEBUTUHAN NON-FUNGSIONAL (NF-01 sd NF-06) DARI DOKUMEN SRS DI SINI]"
el
pp "NF-01 Keamanan (Security): Melindungi kerahasiaan data profil pengguna, password (enkripsi Bcrypt), dan validasi sesi login. Prioritas: Tinggi."
pp "NF-02 Ketersediaan (Availability): Sistem harus dapat diakses 24/7 agar Member dapat melakukan Booking Online kapan saja. Prioritas: Tinggi."
pp "NF-03 Akurasi Perhitungan: Menjamin ketepatan perhitungan billing otomatis berdasarkan durasi main dan tarif unit secara real-time. Prioritas: Tinggi."
...
pp "NF-06 Performa (Performance): Waktu respon sistem saat memvalidasi ketersediaan unit tidak boleh lebih dari 3 detik. Prioritas: Sedang."
...
pp "Skenario Utama:"
li "1. Aktor membuka aplikasi/web."
li "2. Sistem menampilkan formulir login (Username dan Password)."
li "3. Aktor memasukkan kredensial dan menekan tombol login."
li "4. Sistem memvalidasi data ke database."
li "5. Sistem mengarahkan aktor ke dashboard sesuai hak akses (peran) masing-masing."
el
bd "2. Use Case: Log Out"
pp "Aktor: Admin, Member, Pemilik"
pp "Deskripsi: Proses keluar dari sesi sistem secara aman."
pp "Skenario Utama:"
li "1. Aktor menekan tombol Log Out pada menu navigasi."
li "2. Sistem menghapus sesi (session) aktif aktor."
li "3. Sistem mengarahkan kembali ke halaman utama atau halaman login."
el
bd "3. Use Case: Kelola Inventaris"
pp "Aktor: Admin"
pp "Deskripsi: Mengatur data perangkat (PS4/PS5) yang tersedia."
pp "Skenario Utama:"
li "1. Admin memilih menu Kelola Inventaris."
li "2. Sistem menampilkan daftar unit yang ada."
li "3. Admin dapat menambah, mengubah status (baik/rusak), atau menghapus data unit."
li "4. Admin menyimpan perubahan."
li "5. Sistem memperbarui data inventaris di database."
el
bd "4. Use Case: Booking Online"
pp "Aktor: Member"
pp "Deskripsi: Melakukan reservasi jadwal main secara mandiri."
pp "Skenario Utama:"
li "1. Member masuk ke menu Booking Online."
li "2. Member memilih jenis unit (PS4/PS5) dan jadwal (tanggal/jam)."
li "3. Sistem mengecek ketersediaan unit pada waktu tersebut secara otomatis."
li "4. Member mengonfirmasi pemesanan."
li '5. Sistem menerbitkan kode atau status reservasi "Menunggu Konfirmasi/Berhasil".'
el
bd "5. Use Case: Kelola Reservasi"
pp "Aktor: Admin"
pp "Deskripsi: Mengelola dan memproses data booking yang masuk dari member."
pp "Skenario Utama:"
li "1. Admin masuk ke menu Kelola Reservasi."
li "2. Sistem menampilkan daftar pesanan masuk dari Member."
li "3. Admin melakukan verifikasi/validasi data booking."
li "4. Admin mengubah status reservasi (Konfirmasi/Dibatalkan/Selesai)."
li "5. Sistem memperbarui status jadwal agar tidak bentrok dengan member lain."
el
bd "6. Use Case: Monitoring Status Unit"
pp "Aktor: Admin, Member"
pp "Deskripsi: Memantau sisa waktu bermain dan status penggunaan unit secara real-time."
pp "Skenario Utama:"
li "1. Aktor membuka dashboard monitoring."
li "2. Sistem menampilkan status unit (Kosong, Terisi, atau Dipesan)."
li "3. Sistem menjalankan timer otomatis untuk unit yang sedang digunakan."
li "4. Sistem memberikan peringatan/notifikasi jika waktu bermain hampir habis."
li "5. (Khusus Admin) Admin dapat menghentikan atau menambah durasi billing."
el
...
pp "Skenario Utama:"
li "1. Aktor masuk ke menu Laporan Pendapatan."
li "2. Aktor menentukan filter periode (Harian/Bulanan)."
li '3. Sistem menarik data transaksi yang sudah berstatus "Selesai".'
li "4. Sistem menampilkan total pendapatan dan detail transaksi."
li "5. Aktor dapat mengunduh atau mencetak laporan tersebut."
...
# ================================================================ BAB IV (BARU - sesuai template)
pb
h1 "BAB IV"
h1 "PERANCANGAN SISTEM"

h2 "4.1 Perancangan Arsitektur Sistem"
pp "Sistem GO PLAY dirancang menggunakan pola arsitektur MVC (Model-View-Controller) yang diimplementasikan melalui framework Laravel 12. Arsitektur ini memisahkan aplikasi menjadi tiga komponen utama:"
pp "Model: Bertanggung jawab atas pengelolaan data dan interaksi dengan database SQLite. Model utama yang digunakan meliputi: User (data Member), Admin (data Admin), Console (data unit PS), Booking (data reservasi), dan Transaction (data transaksi selesai)."
pp "View: Bertanggung jawab atas tampilan antarmuka pengguna yang dibangun menggunakan Blade Template Engine Laravel dan dikompilasi menggunakan Vite. Setiap peran pengguna (Member, Admin, Pemilik) memiliki tampilan dashboard yang berbeda sesuai hak aksesnya."
pp "Controller: Bertanggung jawab atas logika bisnis aplikasi, memproses HTTP request dari pengguna, berinteraksi dengan Model, dan mengembalikan respons ke View. Controller dipisahkan berdasarkan peran: AuthController, MemberController (BookingController, DashboardController), AdminController (BookingAdminController, ConsoleController, ReportController), dan OwnerController."
...
pp "Basis data sistem GO PLAY dirancang menggunakan SQLite dengan pendekatan relasional. Struktur tabel dibuat menggunakan fitur Migrations Laravel. Entitas utama yang terlibat beserta atributnya adalah:"
el
bd "Tabel users (Data Member/Pelanggan):"
pp "id (PK, auto-increment), name (varchar), email (varchar, unique), password (varchar, encrypted), phone (varchar), email_verified_at (timestamp, nullable), remember_token (varchar, nullable), created_at (timestamp), updated_at (timestamp)."
el
bd "Tabel admins (Data Admin):"
pp "id (PK, auto-increment), name (varchar), email (varchar, unique), password (varchar, encrypted), remember_token (varchar, nullable), created_at (timestamp), updated_at (timestamp)."
el
bd "Tabel consoles (Data Unit PS4/PS5):"
pp "id (PK, auto-increment), name (varchar), type (enum: PS4/PS5), status (enum: available/occupied/maintenance), price_per_hour (integer), description (text, nullable), created_at (timestamp), updated_at (timestamp)."
el
bd "Tabel bookings (Data Reservasi):"
pp "id (PK, auto-increment), booking_code (varchar, unique), user_id (FK ke users.id, nullable untuk walk-in), console_id (FK ke consoles.id), booking_date (date), start_time (time), end_time (time), duration (integer, dalam jam), total_price (integer), status (enum: pending/confirmed/completed/cancelled), payment_status (enum: unpaid/paid), created_at (timestamp), updated_at (timestamp)."
el
bd "Tabel transactions (Data Transaksi Selesai):"
pp "id (PK, auto-increment), booking_id (FK ke bookings.id), amount (integer), payment_method (varchar), paid_at (timestamp), created_at (timestamp), updated_at (timestamp)."
...
pp "Desain antarmuka pengguna sistem GO PLAY dirancang berdasarkan prinsip kemudahan penggunaan (usability) dan estetika visual yang modern. Desain awal (wireframe) dibuat menggunakan Figma sebelum diimplementasikan ke dalam kode program."
pp "Link Figma: https://www.figma.com/site/S3ShQKDKt7BylYmXNlHTX7/GOPLAY?node-id=0-1"
...
pp "Database sistem GO PLAY diimplementasikan menggunakan SQLite. Seluruh struktur tabel dibuat menggunakan fitur Migrations Laravel yang memungkinkan proses pembuatan ulang dan pengelolaan versi database secara sistematis. Untuk menginisialisasi database dengan data awal (seeder), digunakan perintah berikut:"
pp "php artisan migrate --seed"
pp "Seeder yang disediakan meliputi: data Admin default (email: admin@goplay.com), data unit konsol PS4/PS5 (5 unit), dan data Member contoh untuk keperluan pengujian sistem."

h2 "5.2 Implementasi Antarmuka"
pp "Berikut adalah implementasi antarmuka pengguna sistem GO PLAY yang telah berhasil dibangun berdasarkan wireframe yang telah dirancang sebelumnya:"
...
pp "Source code sistem GO PLAY dikelola menggunakan sistem kontrol versi Git dan dipublikasikan pada repositori GitHub:"
pp "https://github.com/adamme26/Sistem-Informasi-Manajemen-dan-Reservasi-Rental-PlayStation-GO-PLAY-Berbasis-Web"
el
pp "Struktur direktori utama sistem GO PLAY:"
...
pp "Pengujian dilakukan secara manual dengan cara menjalankan aplikasi dan mencoba setiap fitur satu per satu untuk memastikan output yang dihasilkan sesuai dengan yang diharapkan berdasarkan spesifikasi kebutuhan yang telah ditetapkan."
el
bd "b. Pengujian Error Handling"
pp "Pengujian dilakukan dengan memasukkan data yang tidak valid untuk memastikan sistem menangani error dengan benar. Contoh kasus yang diuji:"
pp "1. Input durasi berupa teks bukan angka (menyebabkan error Carbon::rawAddUnit() TypeError - diperbaiki dengan casting (int) $request->duration)."
pp "2. Pemesanan pada unit yang sudah terisi atau dalam status maintenance (sistem menampilkan pesan error ketersediaan)."
pp "3. Login dengan kredensial yang salah (sistem menampilkan pesan error autentikasi)."
...
pp "User Acceptance Testing (UAT) dilakukan dengan melibatkan pengguna nyata (Admin dan Member GO PLAY) untuk memastikan bahwa sistem yang dikembangkan telah memenuhi kebutuhan dan ekspektasi pengguna akhir. UAT dilakukan berdasarkan skenario pengujian yang disusun dari setiap use case yang telah didefinisikan."
el
...
h2 "6.1 Kesimpulan"
pp "Berdasarkan hasil analisis, perancangan, implementasi, dan pengujian yang telah dilakukan terhadap Sistem Informasi Manajemen dan Reservasi Rental PlayStation (GO PLAY), dapat ditarik kesimpulan sebagai berikut:"
pp "1. Sistem Informasi GO PLAY telah berhasil dikembangkan menggunakan framework Laravel 12 (PHP 8.2) dengan arsitektur MVC, database SQLite, dan antarmuka Vite + Blade Template Engine."
pp "2. Sistem berhasil mengimplementasikan tujuh fitur fungsional utama sesuai dokumen SRS: Login multi-peran (F-01), Logout (F-02), Kelola Inventaris (F-03), Booking Online (F-04), Kelola Reservasi termasuk Walk-in (F-05), Monitoring Status Unit dengan Timer Otomatis (F-06), dan Laporan Pendapatan (F-07)."
pp "3. Fitur Booking Online (F-04) berhasil mencegah terjadinya double booking melalui mekanisme pengecekan ketersediaan jadwal dan unit secara otomatis sebelum reservasi dikonfirmasi."
pp "4. Fitur Timer Otomatis pada dashboard Admin (F-06) berhasil memantau durasi bermain pelanggan secara real-time dan memberikan notifikasi saat waktu hampir habis, sehingga mengurangi beban kerja Admin secara signifikan."
pp "5. Dashboard Laporan Pendapatan (F-07) berhasil menyajikan rekapitulasi keuangan yang akurat, dapat difilter berdasarkan periode harian maupun bulanan, sehingga memudahkan Pemilik dalam melakukan audit keuangan."
pp "6. Sistem telah diuji melalui pengujian fungsional Black Box Testing dan User Acceptance Testing (UAT), dengan hasil menunjukkan seluruh fitur berjalan sesuai dengan spesifikasi kebutuhan yang telah ditetapkan."

h2 "6.2 Saran"
pp "Untuk pengembangan sistem GO PLAY ke depannya, beberapa saran yang dapat dipertimbangkan:"
pp "1. Integrasi Payment Gateway: Menambahkan fitur pembayaran digital (seperti Midtrans, QRIS, atau Virtual Account) agar proses transaksi dapat diselesaikan secara online tanpa harus membayar di tempat."
pp "2. Notifikasi WhatsApp/Email Otomatis: Mengintegrasikan notifikasi via WhatsApp Business API atau email untuk mengirimkan konfirmasi booking dan pengingat jadwal kepada Member secara otomatis."
pp "3. Pengembangan Aplikasi Mobile: Membangun versi aplikasi mobile (Android/iOS) agar Member dapat melakukan booking dan memantau status pesanan melalui smartphone dengan lebih mudah dan nyaman."
pp "4. Deployment ke Server Hosting: Mengoptimalkan sistem untuk di-deploy ke server hosting berbasis Linux dengan database MySQL/PostgreSQL agar sistem dapat diakses secara publik 24/7 tanpa bergantung pada localhost."
pp "5. Migrasi Database ke MySQL: Mengganti SQLite dengan MySQL untuk lingkungan produksi guna mendukung concurrency akses yang lebih tinggi dan kapasitas penyimpanan data yang lebih besar."
pp "6. Fitur Review dan Rating: Menambahkan fitur ulasan dan penilaian dari pelanggan terhadap layanan, untuk membantu Pemilik mengevaluasi dan meningkatkan kualitas layanan secara berkelanjutan."
...
pp "[5]  Fowler, M. (2002). Patterns of Enterprise Application Architecture. Boston: Addison-Wesley Professional."
...
pp "[10] Connolly, T. M., dan Begg, C. E. (2014). Database Systems: A Practical Approach to Design, Implementation, and Management (6th ed.). Harlow: Pearson Education."
...
ph "[TEMPEL SELURUH GAMBAR DIAGRAM DARI DOKUMEN DOKUMEN_UML_GO_PLAY_Kelompok8.pdf DI SINI]"
...
WF "$tmpDir\word\document.xml" $sb.ToString()
